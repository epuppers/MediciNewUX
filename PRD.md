...
- [ ] **1.8 — Extract purple intensity base colors to data object**
  Move `purpleBaseColors` into the mock data structure as `CONFIG_PURPLE_BASE_COLORS` (use CONFIG_ prefix since this is configuration, not mock data). Move `rgbCompanions` mapping alongside it.

- [ ] **1.9 — Collect all MOCK_ and CONFIG_ objects into a single data file**
  Create `js/mock-data.js` containing all MOCK_ and CONFIG_ objects. Add a script tag in index.html before app.js. Remove the objects from app.js.
